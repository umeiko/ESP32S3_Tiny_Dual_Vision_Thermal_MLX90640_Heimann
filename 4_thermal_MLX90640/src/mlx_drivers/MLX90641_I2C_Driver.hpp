...
{           									
	...
	for(int j = 0 ; j < factor ; j++){
		...
		probeWire->endTransmission();
		delayMicroseconds(5);
		probeWire->beginTransmission(slaveAddr);
		
		int written1 = probeWire->write(cmd[0]);
		int written2 = probeWire->write(cmd[1]);
		
		int end = probeWire->endTransmission(0);
		...
		num_bytes = probeWire->requestFrom((int)slaveAddr,num_bytes);		// update number of bytes that were read
		if(!num_bytes) return -1;
		
		for(int i = 0 ; i < num_bytes/2 ; i++){
			if(probeWire->available()){		
				 uint16_t high = probeWire->read()<<8;
				 uint16_t low = probeWire->read();
				 *p++ = high + low;
			}
		}
	}
