...
int MLX90640_I2CRead(uint8_t _deviceAddress, unsigned int startAddress, unsigned int nWordsRead, uint16_t *data)
{
  ...
  {
    probeWire->beginTransmission(_deviceAddress);
    probeWire->write(startAddress >> 8); //MSB
    probeWire->write(startAddress & 0xFF); //LSB
    if (probeWire->endTransmission(false) != 0) //Do not release bus
    {
    ...
    probeWire->requestFrom((uint8_t)_deviceAddress, (uint8_t)numberOfBytesToRead);
    if (probeWire->available())
    {
      for (uint16_t x = 0 ; x < numberOfBytesToRead / 2; x++)
      {
        //Store data into array
        data[dataSpot] = probeWire->read() << 8; //MSB
        data[dataSpot] |= probeWire->read(); //LSB
...
void MLX90640_I2CFreqSet(int freq)
{
  ...
  probeWire->setClock((long)1000 * freq);
}
...
{
  probeWire->beginTransmission((uint8_t)_deviceAddress);
  probeWire->write(writeAddress >> 8); //MSB
  probeWire->write(writeAddress & 0xFF); //LSB
  probeWire->write(data >> 8); //MSB
  probeWire->write(data & 0xFF); //LSB
  if (probeWire->endTransmission() != 0)
  {
    //Sensor did not ACK
