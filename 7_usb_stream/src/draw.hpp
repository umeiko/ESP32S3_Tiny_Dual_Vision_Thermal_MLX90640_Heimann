...
void draw_connecting_screen(){
    ...
    if (!no_signal_shown){
        tft.fillScreen(TFT_BLACK);
        tft.setTextSize(2);
        tft.drawString("CONNECTING", tft.width() / 2 - 20, tft.height() / 2);
        tft.setTextDatum(TL_DATUM); // 恢复默认
        no_signal_shown = true;
    }
}

void draw_streaming_screen(){
   tft.fillScreen(TFT_BLACK);
   tft.setTextSize(2);
   tft.drawString("STREAMMING", tft.width() / 2 - 10, tft.height() / 2);
   tft.setTextDatum(TL_DATUM); // 恢复默认
}
...
void draw_connected_screen(){
    ...
    if (!no_signal_shown){
        tft.fillScreen(TFT_BLACK);
        tft.setTextSize(2);
        tft.drawString("CONNECTED", tft.width() / 2 - 20, tft.height() / 2);
        tft.setTextDatum(TL_DATUM); // 恢复默认
        no_signal_shown = true;
    }
}
