...
void serial_loop(){
    if (Serial.available() > 0) {
        ...
        if (input == "h") {
            ...
            Serial.println("  nmcli                                    - Show WiFi connection status");
            Serial.println("  nmcli dev wifi                           - Scan available WiFi networks");
            Serial.println("  nmcli dev wifi connect <SSID> password <PWD>  - Connect to a WiFi network");
            Serial.println("=========================================\r\n");
        ...
        } else if (input.startsWith("check_camera") || input.startsWith("test_camera")) {
            camera_cli(input);
            
        } else if (input.startsWith("nmcli")) {
            nmcli(input);
